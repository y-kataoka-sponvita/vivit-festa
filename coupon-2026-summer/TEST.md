...
### 利用可能期間（当日限定）
> 既定の利用可能時間は **2026-08-01 11:00〜17:30（JST）**。`demo` で始まるIDは常に使用可。
> 開催日より前の現時点では、`demo` 以外の未使用クーポンは自動的に「期間外（開催前）」になる。

- [ ] **2-9 期間前（開催前）**：`?id=test-500`（demo以外・未使用）を今開く → 金額は出るが「開催当日にご利用いただけます」＋利用可能時間の案内、店舗プルダウン/使用ボタンは出ない
- [ ] **2-10 demoは常時可**：`?id=demo` は期間外でも通常どおり使用できる（店舗選択・使用可）
- [ ] **2-11 期間外の直接使用（サーバー拒否）**：期間外に非demoIDへ直接POST → `OUTSIDE_PERIOD` で拒否、`used` は変わらない
  ```bash
  curl -sL "$API" --data-urlencode "action=useCoupon" --data-urlencode "id=test-post" --data-urlencode "shopId=1"
  # 開催前/終了後に実行 → {"success":false,"error":{"code":"OUTSIDE_PERIOD",...}}
  ```
- [ ] **2-12 当日（本番相当）**：`CONFIG.USABLE_FROM/UNTIL` を一時的に「今」を含む範囲にして再デプロイ → 非demoIDが使用できることを確認（確認後、8/1の値に戻す）
